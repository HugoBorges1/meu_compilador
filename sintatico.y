...
atrib : IDENT[id] ']'indice[idx]'[' '=' arit[at] {
     ...
     int r = vec_read_counts[$id][indexVal];
     ...
     if (strVal == "" && memory_vector_string.count($id)) { 
          if (declared_floats.count($id)) strVal = to_string($at->getDoubleValue());
          else strVal = to_string($at->getIntValue());
     }
...
atrib : IDENT[id] ']'indice[idx]'[' '=' BOOL_T {
     ...
     $$ = new StoreVector($id, $idx, new ConstBoolean(true), indexVal, true, r, l, i);
...
atrib : IDENT[id] ']'indice[idx]'[' '=' BOOL_F {
     ...
     $$ = new StoreVector($id, $idx, new ConstBoolean(false), indexVal, true, r, l, i);
} 
...
varshow : '%' IDENT[id] ']' atstring[ats] '[' '\\' {
     ...
     int reads = vec_read_counts[$id][idx]; 
     int loops = vec_loop_counts[$id][idx];
     int ifs = vec_if_counts[$id][idx];
     ...
     if (memory_vector_string.count($id) && memory_vector_string[$id].count(idx)) {
          ...
          $$ = new LoadVector($id, $ats, idx, val, true, loops, ifs);
     }
     else {
